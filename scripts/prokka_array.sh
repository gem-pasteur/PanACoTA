...
if [ ! -f $oriname-gembase.fna ]; then
	echo $oriname-gembase.fna "(from original file '$oriname') not found. Annotation and formatting pipeline cannot run." | tee -a $errfile -a $logfile
	exit 1
fi
...
if [ ! $gemname == "gembase_name" ]; then
	...
	if [ -d $oriname-prokka11Res ] && [ -z $force ]; then
		echo "/!\\" `date +"%d/%m/%y - %T"` $gemname $oriname  "WARNING: prokka results folder already exists. Prokka will not run again, formatting step will use already generated results of Prokka in " $oriname-prokka11Res. "If you want to re-run prokka, first remove this result folder, or use '-f' option if you want to rerun prokka for all genomes." | tee -a $errfile -a $logfile
	fi
